#' @title Subset and Fit ZINB Models Pipeline
#'
#' @description This convenience function combines subsetting and zero-inflated negative
...
AnalyzeWithZINB <- function(seuratObject,
  ...
  # ============================================================================
  # 1) SUBSET SEURAT OBJECT BY METADATA
  # ============================================================================
  step1_start <- Sys.time()
  if (verbose) {
    message("Subsetting Seurat object by metadata columns...")
  }
  ...
  if (verbose) {
    ...
    message("\nFitting zero-inflated negative binomial models for each subset...")
  }

  # ============================================================================
  # 2) FIT ZINB MODELS (PARALLEL OR SEQUENTIAL)
  # ============================================================================
  ...
  if (parallel) {
    ...
    model_params_list <- future.apply::future_lapply(
      ...
      function(subset_name) {
        # Get saturation vector for this subset if available
        saturation_vec <- if (!is.null(subset_result$saturation_vectors)) {
          subset_result$saturation_vectors[[subset_name]]
        } else {
          NULL
        }
  ...
  if (verbose) {
    ...
    message("\nMerging results and creating keys...")
  }

  # ============================================================================
  # 3) MERGE RESULTS AND CREATE KEY-BASED OUTPUT
  # ============================================================================
